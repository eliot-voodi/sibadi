...
3. Отредактируйте `.env`: задайте надёжные `DB_PASS` и `SECRET_KEY`. Первый администратор создаётся автоматически:
   - **`ADMIN_BOOTSTRAP_ENABLED=false`** (значение по умолчанию): пароль генерируется криптостойко по требованиям ГОСТ Р 71753 / рекомендациям ФСТЭК (длина ≥12, четыре класса символов) и **один раз** выводится в консоль процесса (`docker compose logs web` / stderr). Пароль в переменных окружения не задаётся.
   - **`ADMIN_BOOTSTRAP_ENABLED=true`**: задайте пароль явно — `ADMIN_BOOTSTRAP_PASSWORD=...` (для сценариев CI/воспроизводимого развёртывания).
   После первого успешного входа при использовании явного bootstrap рекомендуется выставить `ADMIN_BOOTSTRAP_ENABLED=false` и перезапустить контейнер `web`.
...
| `ADMIN_BOOTSTRAP_ENABLED` | `false` — автосоздание admin с паролем по ГОСТ/ФСТЭК и одноразовым выводом в stderr; `true` — пароль только из `ADMIN_BOOTSTRAP_PASSWORD`. |
| `ADMIN_BOOTSTRAP_LOGIN` | Логин первого администратора (по умолчанию `admin`). |
| `ADMIN_BOOTSTRAP_PASSWORD` | Пароль при `ADMIN_BOOTSTRAP_ENABLED=true`; при `false` не используется. |
...
| 443 | HTTPS, прокси на Flask (:8080 внутри сети) |
...
Строк пользователей в `init-db.sql` нет: первый вход — после автосоздания администратора (см. `ADMIN_BOOTSTRAP_*`) либо после ручной вставки в таблицу `"Пользователи"`.
